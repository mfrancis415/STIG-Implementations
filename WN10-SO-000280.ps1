...
$admins = Get-LocalUser | Where-Object { $_.Enabled -eq $true }
if (-not $admins) {
...
$lapsRegPath = "HKLM:\SOFTWARE\Policies\Microsoft Services\LAPS"
if (Test-Path $lapsRegPath) {
    $lapsSettings = Get-ItemProperty -Path $lapsRegPath
    Write-Host "`n=== LAPS Policy Settings ==="
    Write-Host "PasswordSettingsEnabled: $($lapsSettings.Enabled)"
    Write-Host "PasswordComplexity: $($lapsSettings.PasswordComplexity)"
    Write-Host "PasswordLength: $($lapsSettings.PasswordLength)"
    Write-Host "PasswordAgeDays: $($lapsSettings.PasswordAgeDays)"

    $compliance = $true

    if ($lapsSettings.Enabled -ne 1) {
        Write-Host "FINDING: LAPS not enabled." -ForegroundColor Red
        $compliance = $false
    }

    if ($lapsSettings.PasswordComplexity -ne 4) {
        Write-Host "FINDING: Password complexity not set to 4 (uppercase, lowercase, numbers, special chars)." -ForegroundColor Red
        $compliance = $false
    }

    if ($lapsSettings.PasswordLength -lt 14) {
        Write-Host "FINDING: Password length less than 14." -ForegroundColor Red
        $compliance = $false
    }

    if ($lapsSettings.PasswordAgeDays -ne 60) {
...
$lapsLog = Get-WinEvent -LogName "Microsoft-Windows-LAPS/Operational" -ErrorAction SilentlyContinue -MaxEvents 20

if ($null -eq $lapsLog) {
    Write-Host "FINDING: No LAPS operational log found. LAPS may not be installed or logging is disabled." -ForegroundColor Red
} else {
